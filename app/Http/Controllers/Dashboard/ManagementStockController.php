...
class ManagementStockController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Riwayat perubahan stok (stock logs)
        if ($productId) {
            $stockLogs = StockLog::where('product_id', $productId)->with(['product', 'user'])->latest()->paginate(10, ['*'], 'logs_page');
        } else {
            $stockLogs = StockLog::with(['product', 'user'])->latest()->paginate(10, ['*'], 'logs_page');
        }

        return view("dashboard.management-stock.index", compact('products', 'productId', 'stockLogs'));
    }
