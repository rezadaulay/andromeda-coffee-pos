...
@section('content')
  <div class="min-h-screen flex items-center justify-center">
    <div class="w-full max-w-md bg-white p-8 rounded-xl shadow">
            <h1 class="text-2xl font-bold text-center mb-6">Tambah Kategori</h1>
            ...
            <form action="{{ route('product-categories.store') }}" method="POST">
                ...
                <div>
                    ...
                    >
                    @error('name')
                    {{-- jika ada errors dengan object `name` maka masuk ke kondisi --}}
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        {{-- jika ingin print error maka gunakan $message yang akan otomatis mengambil pesan error object `name` --}}
                    @enderror
                </div>
